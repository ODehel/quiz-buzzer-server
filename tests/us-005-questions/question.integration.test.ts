...
} from "../../src/routes/themeRoute.ts";
import { insertQuiz, insertQuizQuestions } from "../../src/repositories/quizRepository.ts";
...
describe("CA-84 — Internal server error on questions", () => {
  test("CA-84 - 500 on unexpected failure in collection handler", async () => {
    ...
    const { createQuestionsCollectionHandler: collHandler } = await import("../../src/routes/questionRoute.ts");
