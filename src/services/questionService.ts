...
import { validateUuid, normalizeName as normalizeTitle, isValidUuid, parsePagination, validateAllowedFields } from "../utils/validation.ts";
 ...
 */
function validateChoices(choices: unknown): string[] {
 ...
 */
function validateCorrectAnswer(correctAnswer: unknown, choices: string[] | null): string {
...
function checkUnknownFields(body: Record<string, unknown>, allowedFields: Set<string>): void {
...
export function createQuestion(db: Database.Database, body: Record<string, unknown>): QuestionApiResponse {
  ...
  return toApiFormat({
    QST_ID: id, QST_TYPE: type as QuestionType, QST_THEME_ID: theme_id, QST_TITLE: title,
 ...
 */
function isIdentical(row: QuestionRow, fields: QuestionUpdateFields): boolean {
