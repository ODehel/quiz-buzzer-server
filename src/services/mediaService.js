...
import { findQuestionById, updateQuestion } from "../repositories/questionRepository.js";
 ...
 */
export function generateMediaFileName(questionId, mediaType, extension) {
 ...
 */
export function generateMediaPath(questionId, mediaType, extension) {
 ...
 * @param {string} mediaPath - Relative path of the media (e.g., '/uploads/questions/...-image.jpg')
 * @returns {Promise<boolean>} true if deleted, false if file doesn't exist
 */
export async function deleteMediaFile(uploadsDir, mediaPath) {
  if (!mediaPath) {
    return false;
  }
 ...
 * @param {string} uploadsDir - Base uploads directory
 * @throws {AppError} 404 NOT_FOUND if question doesn't exist
 * @returns {Object} Updated question object
 */
export async function uploadMedia(db, questionId, mediaType, file, uploadsDir) {
 ...
 * @param {string} mediaType - Type of media ('image' or 'audio')
 * @param {string} uploadsDir - Base uploads directory
 * @throws {AppError} 404 NOT_FOUND if question or media doesn't exist
 * @returns {Promise<void>}
 */
export async function deleteMedia(db, questionId, mediaType, uploadsDir) {
  // Validate question exists
  const question = findQuestionById(db, questionId);
  if (!question) {
    throw new AppError(404, "NOT_FOUND", "The requested question was not found.");
