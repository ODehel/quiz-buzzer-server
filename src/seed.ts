...
export async function runSeed({
  ...
  env,
  generateId = uuidv7,
  hashPassword = (pwd: string) => bcrypt.hash(pwd, Number(process.env.BCRYPT_COST) || 12),
  ...
  const entries: Array<SeedUser & { password: string }> = [];
  ...
  // 3. Hachage (async)
  const hashedEntries: Array<SeedUser & { password: string; hashedPassword: string }> = [];
  ...
  const insertAll = db.transaction((items: typeof hashedEntries) => {
...
if (isMainModule) {
  ...
  try {
    const result = await runSeed({ db, env: process.env as Record<string, string | undefined> });
