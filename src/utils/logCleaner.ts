...
const RETENTION_MS: number = 7 * 24 * 60 * 60 * 1000; // 7 jours
 ...
 */
export async function cleanOldLogs(logsDir: string = "logs", now: number = Date.now()): Promise<number> {
  let deletedCount = 0;
  ...
  try {
    ...
    for (const file of logFiles) {
      ...
      try {
        ...
        logger.warn({ event: "LOGS_CLEANUP_FILE_ERROR", file, error: (err as Error).message });
      }
    }
  } catch {
