...
import { transitionState, resolveCurrentQuestion } from "./gameworkflow.js";
...
import { insertGameAnswer } from "../repositories/gameanswerRepository.js";
import { updateGameStatus } from "../repositories/gameRepository.js";
import { logInfo, logWarn } from "../utils/logger.js";

/**
 * Crée les handlers du workflow SPEED.
 *
 * @param {Object} ctx - Contexte partagé avec l'orchestrateur
 * @returns {Object} Handlers SPEED
 */
export function createSpeedWorkflow(ctx) {

  // ── trigger_title → QUESTION_OPEN directly (CA-4 to CA-7) ─────────────

  function handleTriggerTitle(game, question) {
  ...
  function handleTimerExpire(gameId, questionIndex) {
    ...
    if (game.GAM_STATUS === "QUESTION_BUZZED") {
      ...
      logInfo("GAME_TIMER_EXPIRED_DURING_BUZZ", {
        ...
        participant_order: speedProcessor?.getCurrentBuzzer()?.participantOrder,
      });
  ...
  async function closeQuestionNoWinner(game) {
    const speedProcessor = ctx.getSpeedProcessor();
