...
const TIMER_STATUSES = new Set(["QUESTION_OPEN", "QUESTION_BUZZED"]);
 ...
 * @param {string} role - "admin" ou "buzzer"
 ...
 */
function syncBuzzer(send, game, username, db) {
  ...
  const cumulativeScore = participant
    ? (db.prepare(
      ...
      ).get(game.GAM_ID, participant.GPA_ORDER)?.score ?? 0)
    : 0;
