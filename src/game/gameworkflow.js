...
import { findGameById, updateGameStatus } from "../repositories/gameRepository.js";
 ...
 * @param {string} gameId
 * @param {string} currentStatus - État attendu (doit correspondre à la DB)
 * @param {string} newStatus - État cible
 * @throws {Error} GAME_NOT_FOUND | STATE_MISMATCH | INVALID_TRANSITION
 */
export function transitionState(db, gameId, currentStatus, newStatus) {
 ...
 * @param {import("better-sqlite3").Database} db
 * @param {string} quizId
 * @param {number} questionIndex - Index 0-based
 * @returns {Object|undefined} Ligne complète de T_QUESTION_QST, ou undefined si hors limites
 */
export function resolveCurrentQuestion(db, quizId, questionIndex) {
 ...
 * @param {import("better-sqlite3").Database} db
 * @param {string} quizId
 * @param {number} currentIndex - Index 0-based de la question courante
 * @returns {boolean} true s'il existe une question à currentIndex + 1
 */
export function hasMoreQuestions(db, quizId, currentIndex) {
