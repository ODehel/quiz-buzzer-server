...
interface IntermediateRankingEntry {
  rank: number;
  participant_name: string;
  participant_order: number;
  cumulative_score: number;
 ...
 * - Ajout du rang (1, 2, 3...)
 */
export function calculateIntermediateRanking(db: Database.Database, gameId: string): IntermediateRankingEntry[] {
  const rows = db
    .prepare(
      `SELECT
