 ...
 * @param {string} options.correctAnswer - "A", "B", "C" ou "D"
 * @param {number} options.points - Points attribués pour une bonne réponse
 ...
 * @param {Object<number, number>} options.cumulativeScores - Score cumulé avant cette question par participant
 * @param {Set<number>|null} [options.connectedOrders=null] - Ordres des participants connectés (null = tous)
 */
export function createAnswerProcessor({
  ...
  /** @type {Map<number, { answer: string, timeMs: number }>} */
   ...
   * @param {string|null} answer - "A", "B", "C" ou "D"
   * @param {number} timeMs - Temps de réponse en ms
   * @returns {{ accepted: boolean, participantOrder: number, answer?: string, timeMs?: number, reason?: string }}
   */
  function recordAnswer(participantOrder, answer, timeMs) {
   ...
   * @returns {boolean}
   */
  function hasAnswered(participantOrder) {
   ...
   *
   * @returns {Array<{ questionId: string, participantOrder: number, answer: string|null,
   *                    timeMs: number, correct: boolean, pointsEarned: number, cumulativeScore: number }>}
   */
  function getResults() {
   ...
   *
   * @returns {Array<{ rank: number, participantOrder: number, cumulativeScore: number, totalTimeMs: number }>}
   */
  function getRanking() {
