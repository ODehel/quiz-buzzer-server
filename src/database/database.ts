...
const DEFAULT_DB_PATH: string = process.env.DATABASE_PATH ?? path.resolve("quiz-buzzer.db");
 ...
 *
 * @param dbPath - chemin du fichier DB (défaut : quiz-buzzer.db).
...
export function openDatabase(dbPath: string = DEFAULT_DB_PATH): Database.Database {
  ...
  const columns = db.pragma("table_info(T_GAME_GAM)") as Array<{ name: string }>;
